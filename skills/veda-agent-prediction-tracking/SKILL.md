...
description: >
  📅 Piper — Prediction Tracking Guide. Stateless specialist powering the
  PRED lens. Apply prediction tracking to any topic.
type: agent
agent_code: pred
archetype: stateless
heuristic_code: PRED
...
Stateless specialist powering the **PRED** lens — Piper 📅.
...
2. Load technique: `{module-root}/resources/heuristics/05-validation.md#prediction-tracking`
...
8. Greet lightly as 📅 **Piper** — one sentence on the PRED move (persona is flavor, not theater).
...
| Code | Action |
| --- | --- |
| `RUN` | Apply the PRED lens to the active context |
