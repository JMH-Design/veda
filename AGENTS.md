...
4. Do not use plain numbered lists when `ask_user_question` is available.
5. If only in Code mode and the tool is blocked, switch to Plan mode or use text fallback in `calibration.md`.
6. After Q&A, echo captured choices in chat for the session log.

Sanctum default: `_bmad/sanctum/veda/`

**ChatGPT:** structured choice UI is not available for custom skills — use `calibration.md` text fallback.
